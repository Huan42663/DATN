            ...
            @if ($product != null)
                <div class="heading bora-4">
                    <form action="{{ route('Client.orders.createRate') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row mb-3">
                            <div class="row d-flex" style="height: 250px">
                                <div class="col-2 mb-3 me-2">
                                    <a href="{{ route('Client.product.detail', $product->product_name) }}"><img
                                            class="me-3" src="{{ asset('storage/' . $product->product_image) }}"
                                            alt="" style=" width:100%;height:60%;border-radius:10px;" /></a>
                                </div>
                                <div class="col-9 mb-3">
                                    ...
                                    <div class="product[0]s-center gap-2 duration-300 relative z-[1]">
                                        ...
                                        <p class="fs-5 fw-bold">Tổng Tiền :
                                            @if ($product->sale_price > 0)
                                                {{ number_format($product->sale_price * $product->quantity, 0, ',', '.') . ' VNĐ' }}
                                                <br>
                                            @else
                                                {{ number_format($product->price * $product->quantity, 0, ',', '.') . ' VNĐ' }}
                                                <br>
                                            @endif
                                        </p>
                                    </div>
                                ...
                                <div class="col-7">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control"
                                            placeholder="Đánh giá của bạn về sản phẩm" name="content"
                                            value={{ old('content') }}>
                                        @error('content')
                        ...
                        <button type="submit" class="btn btn-dark bg-dark text-light text-center mb-3"
                            width="50px">Gửi</button>
                    </form>
                </div>
            @endif
        </div>
